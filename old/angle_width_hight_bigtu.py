...
def generate_bezier(P0, P3, k1, k2, d1=None, d2=None, samples_per_unit=1.5):
...
def plot_full_track(a, b, k1, k2, track_width=1.0, via=None, ground_height=0.0):
    ...
    if via:
        mid = via
        pix1, curve1, ctrl1 = generate_bezier(a, mid, k1, 0.0)
        pix2, curve2, ctrl2 = generate_bezier(mid, b, 0.0, k2)
        all_points += pix1 + pix2
        curves = [(curve1, ctrl1), (curve2, ctrl2)]
    else:
        pix, curve, ctrl = generate_bezier(a, b, k1, k2)
        all_points += pix
        curves = [(curve, ctrl)]
...
if __name__ == "__main__":
    ...
    if use_mid == 'y':
        ...
        xm = float(input("xm = "))
        ym = float(input("ym = "))
        plot_full_track((x0, y0), (x1, y1), k1, k2, track_width, via=(xm, ym), ground_height=ground_height)
    else:
        plot_full_track((x0, y0), (x1, y1), k1, k2, track_width, ground_height=ground_height)
